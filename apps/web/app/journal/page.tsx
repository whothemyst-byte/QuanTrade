      ...
      {sections.map(({ book, trades }) => (
        <section key={book.id} className="flex flex-col gap-3">
          <div className="flex items-baseline justify-between border-b border-[--color-border] pb-2">
            ...
            <span className="text-xs text-[--color-muted]">{trades.length} closed</span>
          </div>

          {trades.length === 0 ? (
            <p className="text-sm text-[--color-muted]">No closed trades yet.</p>
          ) : (
            trades.map((t) => {
              const np = netPct(t);
                ...
                >
                  <summary className="cursor-pointer flex items-center justify-between gap-3 list-none">
                    <div className="flex items-center gap-2">
                      ...
                      {t.isShadow && (
                        <span className="text-[10px] uppercase tracking-wide rounded-full border border-[--color-border] px-1.5 py-0.5 text-[--color-muted]">
                          shadow
                        </span>
                      )}
                    ...
                    <span className={`num ${won ? "text-[--color-long]" : "text-[--color-short]"}`}>
                      {signedPct(np)}
                    </span>
                  </summary>
                  ...
                  <div className="mt-4 flex flex-col gap-3 text-sm">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div className="rounded-md bg-[--color-bg] px-2.5 py-1.5">
                        <div className="text-[10px] uppercase text-[--color-muted]">gross</div>
                        <div className="num text-sm">{money(t.grossPnl, book.currency)}</div>
                      </div>
                      <div className="rounded-md bg-[--color-bg] px-2.5 py-1.5">
                        <div className="text-[10px] uppercase text-[--color-muted]">costs</div>
                        <div className="num text-sm">
                          {money(t.entryCosts + t.exitCosts, book.currency)}
                        </div>
                      </div>
                      <div className="rounded-md bg-[--color-bg] px-2.5 py-1.5">
                        <div className="text-[10px] uppercase text-[--color-muted]">net</div>
                        <div className="num text-sm">{money(t.netPnl, book.currency)}</div>
                      </div>
                    </div>

                    <p className="text-xs text-[--color-muted] num">
                    ...
                    {t.postMortem ? (
                      <div className="rounded-md border border-[--color-border] p-3">
                        ...
                        <p className="mt-2">
                          <span className="text-[--color-muted]">Expected:</span>{" "}
                          {t.postMortem.expected}
                        </p>
                        <p>
                          <span className="text-[--color-muted]">Actual:</span> {t.postMortem.actual}
                        </p>
                        <p className="mt-1">
                          <span className="text-[--color-muted]">Lesson:</span> {t.postMortem.lesson}
                        </p>
                      </div>
                    ) : (
