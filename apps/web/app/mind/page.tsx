...
export default async function MindPage() {
  ...
  return (
    <div className="flex flex-col gap-10">
      <div>
        <h1 className="font-[family-name:var(--font-display)] text-3xl">Mind</h1>
      ...
      {!doc ? (
        <p className="text-sm text-[--color-muted]">AGENT.md could not be read.</p>
      ) : (
        <>
          <section>
            <div className="flex items-center gap-2 border-b border-[--color-border] pb-2 mb-3">
              <h2 className="text-lg">Core Mandate</h2>
              <span className="text-[10px] uppercase tracking-wide rounded-full border border-[--color-accent] text-[--color-accent] px-2 py-0.5">
                locked · agent cannot edit
              </span>
            </div>
            <pre className="rounded-lg border-2 border-[--color-accent]/30 bg-[--color-surface] p-4 text-xs whitespace-pre-wrap leading-relaxed text-[--color-muted]">
              {doc.coreMandate}
            </pre>
          </section>

          <section>
            <h2 className="text-lg border-b border-[--color-border] pb-2 mb-3">Market Beliefs</h2>
            {doc.beliefs.length === 0 ? (
              ...
              <ul className="flex flex-col gap-2">
                {doc.beliefs.map((b) => (
                  <li key={b} className="rounded-md bg-[--color-surface] px-3 py-2 text-sm">
                    {b}
                  </li>
                ))}
              </ul>
          ...
          <section>
            <div className="flex items-baseline justify-between border-b border-[--color-border] pb-2 mb-3">
              <h2 className="text-lg">Active Rules</h2>
              <span className="text-xs text-[--color-muted]">{active.length} of 15</span>
            </div>
            {active.length === 0 ? (
              <p className="text-sm text-[--color-muted]">
                ...
                {active.map((r) => (
                  ...
                  >
                    <header className="flex items-start justify-between gap-3">
                      <div>
                        <span className="num text-xs text-[--color-muted]">{r.id}</span>
                        <h3 className="text-sm">{r.title}</h3>
                      </div>
                      <span className="text-[10px] uppercase tracking-wide rounded-full bg-[--color-raised] px-2 py-0.5">
                        {r.status}
                      </span>
                    </header>
                    <p className="mt-2 text-xs text-[--color-muted] num">
          ...
          <section>
            <h2 className="text-lg border-b border-[--color-border] pb-2 mb-3">
              Known Failure Modes
            </h2>
            {doc.failureModes.length === 0 ? (
              <p className="text-sm text-[--color-muted]">None recorded yet.</p>
            ) : (
              <ul className="flex flex-col gap-2">
                {doc.failureModes.map((f) => (
                  <li key={f} className="rounded-md bg-[--color-surface] px-3 py-2 text-sm">
                    {f}
                  </li>
                ))}
              </ul>
          ...
          {retired.length > 0 && (
            <section>
              <h2 className="text-lg border-b border-[--color-border] pb-2 mb-3">Retired Rules</h2>
              <div className="flex flex-col gap-2">
                {retired.map((r) => (
                  <article
                    key={r.id}
                    className="rounded-lg border border-[--color-border] bg-[--color-surface] p-3 opacity-60"
                  >
      ...
      <section>
        ...
        {reflections.length === 0 ? (
          ...
          <ol className="flex flex-col gap-3">
            {reflections.map((r) => (
              <li key={r.id} className="rounded-lg border border-[--color-border] bg-[--color-surface] p-4">
                <header className="flex items-baseline justify-between gap-3">
                  <span className="text-sm">{bookMeta(r.bookId).label}</span>
                  <span className="text-xs text-[--color-muted]">{shortDate(r.createdAt)}</span>
                </header>
                ...
                {r.commitSha && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-2 inline-block text-xs text-[--color-accent] num"
                  >
